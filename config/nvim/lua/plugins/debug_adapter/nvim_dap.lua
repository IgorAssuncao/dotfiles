...
return {
    ...
    config = function()
        ...
        if not status_mason_registry then
            ...
            local mason_codelldb_path = vim.fs.joinpath(vim.fn.expand('$MASON'), 'packages', codelldb_package.name,
                'extension')
            codelldb_path = vim.fs.joinpath(mason_codelldb_path, "adapter/codelldb")
        end
